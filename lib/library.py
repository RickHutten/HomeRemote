...
class Library:
    ...
    def add_album(self, album_object):
        if album_object not in self.get_albums():
            self.albums.append(album_object)

    def add_artist(self, artist_object):
        if artist_object not in self.get_artists(True):
    ...
    def print_lib(self):
        ...
        print ""
        for artist in sorted(self.get_artists(), key=lambda a: a.get_name()):
        ...
        for album in sorted(self.get_albums(), key=lambda a:
