...
def fix():
	...
	if ans == 1:
		...
		for j, artist in enumerate(sorted(library.get_artists(), key=lambda a: a.get_name())):
		...
		yesno = raw_input("Do you want to rename " + 
				  sorted(library.get_artists(), key=lambda a: a.get_name())[i].get_name() + "? (Y/N) ")
		if yesno == "Y" or yesno == "y":
			new_name = raw_input("What do you want to be the new name of the artist? ")
			artistObj = sorted(library.get_artists(), key=lambda a: a.get_name())[i]
			songs = artistObj.get_songs()
			for song in songs:
	...
	if ans == 2:
		...
		for j, album in enumerate(sorted(library.get_albums(), key=lambda a: a.get_title())):
		...
		if i == j+1:
			...
			return
		yesno = raw_input("Do you want to rename " + 
				  sorted(library.get_albums(), key=lambda a: a.get_title())[i].get_title() + "? (Y/N) ")
		if yesno == "Y" or yesno == "y":
			new_name = raw_input("What do you want to be the new name of the album? ")
			albumObj = sorted(library.get_albums(), key=lambda a: a.get_title())[i]
			songs = albumObj.get_songs()
			for song in songs:
				audio = MP3(song.get_path(), ID3=EasyID3)
				...
				nr_files += 1
			print "Files renamed:", nr_files
			print "Changes will become visible next time you run 'refreshlib.py'"
		ask_continue()
	...
	if ans == 3:
		...
		print "The following albums have been found:"
		for j, album in enumerate(sorted(artist.get_albums(), key=lambda a: a.get_title())):
			print str(j) + ". " + album.get_title()
		print str(j+1) + ". None, go back"
		...
		for j, song in enumerate(sorted(album.get_songs(), key=lambda a: a.get_title())):
		...
		yesno = raw_input("Do you want to rename " + song.get_title() + "? (Y/N) ")
		if yesno == "Y" or yesno == "y":
			new_name = raw_input("What do you want to be the new name of the song? ")
			audio = MP3(song.get_path(), ID3=EasyID3)
